/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { OutreachPartnerWhereInput } from "./OutreachPartnerWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { OutreachPartnerOrderByInput } from "./OutreachPartnerOrderByInput";

@ArgsType()
class OutreachPartnerFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => OutreachPartnerWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => OutreachPartnerWhereInput, { nullable: true })
  @Type(() => OutreachPartnerWhereInput)
  where?: OutreachPartnerWhereInput;

  @ApiProperty({
    required: false,
    type: [OutreachPartnerOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [OutreachPartnerOrderByInput], { nullable: true })
  @Type(() => OutreachPartnerOrderByInput)
  orderBy?: Array<OutreachPartnerOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { OutreachPartnerFindManyArgs as OutreachPartnerFindManyArgs };
